...
import type { vote as VoteRoute } from "./vote.routes";
  ...
  if (data.type === "FREE") {
    ...
    if (profile && profile.lastFreeVoteAt) {
      ...
      if (diff < 24 * 60 * 60 * 1000) {
        return c.json({
          error: {
            issues: [{ code: "TOO_MANY_REQUESTS", path: ["type"], message: "You can only use a free vote once every 24 hours for this contest." }],
