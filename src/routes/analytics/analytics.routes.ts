...
export const getVotesAnalytics = createRoute({
  path: "/analytics/votes",
  method: "get",
  tags,
  summary: "Get Votes Analytics",
  description: "Get vote statistics including total votes, free votes, paid votes, top voters, and top vote recipients",
  responses: {
    [HttpStatusCodes.OK]: jsonContent(
      z.object({
        totalVotes: z.number().describe("Total number of votes cast"),
        ...
        topVoters: z.array(
          z.object({
            ...
            profileImage: z.string().describe("The voter's profile image"),
            totalVotesGiven: z.number().describe("Total number of votes given by this user"),
          }),
        ).describe("Top 5 users who have given the most votes"),
        topVoteRecipients: z.array(
          z.object({
            profileId: z.string().describe("The vote recipient's profile ID"),
            username: z.string().describe("The vote recipient's username"),
            name: z.string().describe("The vote recipient's name"),
            profileImage: z.string().describe("The vote recipient's profile image"),
            totalVotesReceived: z.number().describe("Total number of votes received by this user"),
          }),
        ).describe("Top 5 users who have received the most votes"),
      }),
      "Votes analytics data",
    ),
    [HttpStatusCodes.UNAUTHORIZED]: UnauthorizedResponse(),
  },
});
...
export type GetVotesAnalyticsRoute = typeof getVotesAnalytics;
